...
function DeleteField({ open, setOpen,item ,data,setData}) {
    if (!open) return null;
            ...
            <div className="w-full max-w-md mx-4">
                <div className="bg-white rounded-xl shadow-xl overflow-hidden">
                    <div className="bg-gradient-to-r from-[#9DDA68FF] to-[#749F4DFF] p-5">
                        <h2 className="text-2xl font-bold text-white text-center">Հաստատել ջնջումը</h2>
                    </div>

                    <div className="p-6">
                        <p className="text-gray-700 text-lg text-center mb-6">
                            Դուք համոզված եք, որ ցանկանում եք ջնջել «<span className={'font-bold'}>{item.title}</span>» դասընթացը?
                        </p>
...
active:border-b-[2px] active:brightness-90 active:translate-y-[2px]"                            >
                            ...
                            <button
                                onClick={() => {
                                    setData((prev) => prev.map((course) => course.id === item.id ? {...course, del: true} : course))
                                    setOpen(false);
                                }}
